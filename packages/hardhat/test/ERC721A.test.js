...
describe("ERC721A", function () {
  beforeEach(async function () {
    ...
    this.NotAnERC721Receiver = await ethers.getContractFactory(
      "NotAnERC721ReceiverMock"
    );
  ...
  context("with minted tokens", async function () {
    ...
    describe("_numberMinted", async function () {
      it("returns the amount for a given address", async function () {
        expect(await this.erc721a.numberMinted(this.owner.address)).to.equal(
          "0"
        );
        expect(await this.erc721a.numberMinted(this.addr1.address)).to.equal(
          "1"
        );
        expect(await this.erc721a.numberMinted(this.addr2.address)).to.equal(
          "2"
        );
        expect(await this.erc721a.numberMinted(this.addr3.address)).to.equal(
          "3"
        );
      });
    });
    ...
    describe("ownerOf", async function () {
      ...
      it("reverts for an invalid token", async function () {
        await expect(this.erc721a.ownerOf(10)).to.be.revertedWith(
    ...
    describe("approve", async function () {
      ...
      it("sets approval for the target address", async function () {
        await this.erc721a
          .connect(this.addr1)
          .approve(this.addr2.address, tokenId);
      ...
      it("does not get approved for invalid tokens", async function () {
        await expect(this.erc721a.getApproved(10)).to.be.revertedWith(
    ...
    describe("setApprovalForAll", async function () {
      it("sets approval for all properly", async function () {
        const approvalTx = await this.erc721a.setApprovalForAll(
          this.addr1.address,
          true
        );
        ...
        expect(
          await this.erc721a.isApprovedForAll(
            this.owner.address,
            this.addr1.address
          )
        ).to.be.true;
    ...
    context("tokenURI", async function () {
      it("tokenURI is always an empty string before setBaseURI is called", async function () {
        const uri = await this.erc721a.tokenURI(0);
    ...
    context("test transfer functionality", function () {
      const testSuccessfulTransfer = function (transferFn) {
        ...
        beforeEach(async function () {
          ...
          from = sender.address;
          this.receiver = await this.ERC721Receiver.deploy(
        ...
        it("clears the approval for the token ID", async function () {
          expect(await this.erc721a.getApproved(tokenId)).to.be.equal(
            ZERO_ADDRESS
          );
        });
      ...
      const testUnsuccessfulTransfer = function (transferFn) {
        ...
        it("rejects unapproved transfer", async function () {
          await expect(
            this.erc721a
              .connect(this.addr1)
              [transferFn](this.addr2.address, this.addr1.address, tokenId)
        ...
        it("rejects transfer from incorrect owner", async function () {
          ...
          await expect(
            this.erc721a
              .connect(this.addr1)
              [transferFn](this.addr3.address, this.addr1.address, tokenId)
        ...
        it("rejects transfer to zero address", async function () {
          ...
          await expect(
            this.erc721a
              .connect(this.addr1)
              [transferFn](this.addr2.address, ZERO_ADDRESS, tokenId)
  ...
  context("mint", async function () {
    beforeEach(async function () {
      ...
      this.badreceiver = await this.NotAnERC721Receiver.deploy(
        RECEIVER_MAGIC_VALUE
      );
    });
    ...
    describe("safeMint", function () {
      it("successfully mints a single token", async function () {
        const mintTx = await this.erc721a["safeMint(address,uint256)"](
      ...
      it("successfully safeMints a single token with data", async function () {
        const mintTx = await this.erc721a["safeMint(address,uint256,bytes)"](
          this.receiver.address,
          1,
          0x42
        );
      ...
      it("fails to safeMint a single token with data to a bad receiver", async function () {
        await expect(
          this.erc721a["safeMint(address,uint256,bytes)"](
            this.badreceiver.address,
            1,
            0x42
          )
      ...
      it("successfully mints multiple tokens", async function () {
        ...
        for (let tokenId = 0; tokenId < 5; tokenId++) {
          ...
          expect(await this.erc721a.ownerOf(tokenId)).to.equal(
            this.receiver.address
          );
        }
      });

      it("successfully mints max # tokens", async function () {
        const mintTx = await this.erc721a["safeMint(address,uint256)"](
          this.receiver.address,
          10
        );
      ...
      it("requires quantity to be greater than 0", async function () {
        await expect(
          this.erc721a["safeMint(address,uint256)"](this.owner.address, 0)
        ).to.be.revertedWith("MintZeroQuantity");
      });

      it("fails to mint too many tokens", async function () {
        await expect(
          this.erc721a["safeMint(address,uint256)"](this.owner.address, 11)
        ).to.be.revertedWith("MintTooManyTokens");
      });

      it("reverts for non-receivers", async function () {
        const nonReceiver = this.erc721a;
        await expect(
          this.erc721a["safeMint(address,uint256)"](nonReceiver.address, 1)
        ).to.be.revertedWith("TransferToNonERC721ReceiverImplementer");
      });
    });
    ...
    describe("mint", function () {
      ...
      it("successfully mints a single token", async function () {
        const mintTx = await this.erc721a.mint(
          this.receiver.address,
          1,
          data,
          false
      ...
      it("rejects mints to the zero address", async function () {
        await expect(
          this.erc721a.mint(ZERO_ADDRESS, 1, data, false)
        ).to.be.revertedWith("MintToZeroAddress");
      });

      it("requires quantity to be greater than 0", async function () {
        await expect(
          this.erc721a.mint(this.owner.address, 0, data, false)
        ).to.be.revertedWith("MintZeroQuantity");
      });
    });
  });
